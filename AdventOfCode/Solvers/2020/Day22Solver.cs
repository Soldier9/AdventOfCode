namespace AdventOfCode.Solvers.Year2020
{
    class Day22Solver : AbstractSolver
    {
        ...
        {
            Dictionary<int, LinkedList<int>> decks = new();
            ...
            using (StreamReader input = File.OpenText(InputFile))
            {
                ...
                while (!input.EndOfStream)
                {
                    ...
                    {
                        _ = decks[currentPlayer].AddLast(int.Parse(line));
                    }
                }
            }
            ...
            while (decks[1].Count > 0 && decks[2].Count > 0)
            {
                ...
                {
                    _ = decks[1].AddLast(decks[1].First!.Value);
                    _ = decks[1].AddLast(decks[2].First!.Value);
                }
                else
                {
                    _ = decks[2].AddLast(decks[2].First!.Value);
                    _ = decks[2].AddLast(decks[1].First!.Value);
                }
                decks[1].RemoveFirst();
                decks[2].RemoveFirst();
            }
        ...
        private int RecursiveCombat(Dictionary<int, LinkedList<int>> decks)
        {
            ...
            while (decks[1].Count > 0 && decks[2].Count > 0)
            {
                ...
                {
                    Dictionary<int, LinkedList<int>> nextDeck = new();
                    ...
                    for (int n = 0; n < Math.Max(card1, card2); n++)
                    {
                        ...
                        if (n < card1)
                        {
                            _ = nextDeck[1].AddLast(d1!.Value);
                            d1 = d1.Next;
                        }
                        if (n < card2)
                        {
                            _ = nextDeck[2].AddLast(d2!.Value);
                            d2 = d2.Next;
                        }
                    }
                    ...
                    if (recWinner == 1)
                    {
                        _ = decks[1].AddLast(card1);
                        _ = decks[1].AddLast(card2);
                    }
                    else
                    {

                        _ = decks[2].AddLast(card2);
                        _ = decks[2].AddLast(card1);
                    }
                }
                else
                {
